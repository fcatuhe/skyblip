...
Sizes come from the device, never from a constant. An SMP packet is as long as the `buf_size` the OS group's parameters read reports (`CONFIG_MCUMGR_TRANSPORT_NETBUF_SIZE`), and it leaves in 20-byte writes, because `CONFIG_MCUMGR_TRANSPORT_BT_REASSEMBLY` puts a packet back together from any number of writes and Web Bluetooth exposes no MTU to write more. Replies come back cut at the device's notification payload, and `smp.js` reassembles them on the length in the header. The first upload packet carries the image's SHA-256: img_mgmt answers a second upload of the same image with the offset it reached, so a window that closed mid-image resumes rather than restarts, as long as the device has not rebooted.
