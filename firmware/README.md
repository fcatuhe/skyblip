...
`make test` runs the host suite, `make test-sanitize` runs it again under ASan and UBSan in `build/sanitize/`, `make fuzz` runs the libFuzzer harnesses in `test/fuzz/` with clang, `make simulator` builds the terminal one, `scripts/build_local.sh` from the repo root builds the device image.
