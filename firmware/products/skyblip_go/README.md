...
Migration only runs forward, and a layout cannot be read backwards: fields were removed and inserted mid-struct, so an older layout is not a prefix of a newer one. The image the bootloader reverts to meets exactly that, a blob its successor wrote. `ConfigLinkService::load()` tells a newer layout from a torn sector by the CRC over the whole stored length (`settings::sealed`), and keeps the newer one untouched: from then on this image reads and writes `settings.prior` instead, so the successor finds its blob again if it is installed a second time. `record_update()` writes the settings in use to `settings.prior` on the way into a swap, so a revert comes back up on the settings the pilot had, not on defaults. Whatever a load fell back to is said: `"settings":"prior"` or `"settings":"defaults"` on the `update` frame a phone is pushed on connecting, and `DEFAULTS` on the self test's `STORAGE` row. A torn sector falls back to defaults and is written over by the next change.
